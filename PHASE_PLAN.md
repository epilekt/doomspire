# 1) Базовое оружие и старт-набор игрока

## Регистрируем предметы ✅
**Файл:** `grimfate/src/main/java/com/doomspire/grimfate/registry/ModItems.java`

**Оружие/щит:**
- `rusty_sword`, `copper_sword`
- `weathered_staff`, `apprentice_staff`
- `simple_bow`, `hunters_bow`
- `weathered_shield`

**Материалы (для крафтов):**
- `hardwood_shaft`, `linen_cloth`, `copper_rivet` (пока простые items)

**Компоненты данных (item data components):** подключаем уже используемые `stat_bonus`, позже добавим `element_bonus`, `class_tag`.
...
- `staves.json`
- `ranged_weapons.json`
- `daggers.json` (под ассасина)
- (опционально) `two_handed.json`

*Позже сделаем «мост» к Better Combat: если мод найден, читаем/мэппим их теги в наши.*

## Рецепты (пока ванильные) ✅
**Директория:** `data/grimfate/recipes/*.json`
- `copper_sword.json`, `apprentice_staff.json`, `hunters_bow.json`, `weathered_shield.json`

*Материалы включаем в рецепты, но стартовые (`rusty_sword`, `weathered_staff`, `simple_bow`) без рецептов (или с дорогими, чтобы не плодить дубликаты).*

## Выдача старт-набора игроку ✅
**Файл:** `grimfate/src/main/java/com/doomspire/grimfate/server/StartKitHandler.java`

- Подписка на `PlayerEvent.PlayerLoggedInEvent`.
- Проверка флага в `PlayerProgressAttachment` (`hasReceivedStartKit`).
- Выдаём: `weathered_staff`, `rusty_sword`, `simple_bow` + базовые стрелы/немного еды.
- Сохраняем флаг, синк на клиент.

## ПКМ-автоатака «Bolt» для посохов ✅
В `apprentice_staff`/`weathered_staff` задаём поведение ПКМ: если удерживается ПКМ — «стреляем» базовым Bolt-снарядом с микро-манакостом и малым уроном.
...
- **«Apprentice»** (маг): +SPIRIT/+INT, небольшой `mana_regen`
- **«Scout»** (плут): +DEX/+EVASION, `move_speed`
- **«Militia»** (воин): +STR/+VIT, немного `damage_reduction`

*Сет-бонус (2 шт. и 4 шт.) считаем в `StatCalculator`: пробег по слотам, считаем теги предметов `grimfate:set/apprentice` и т.д., применяем бонусы на пересчёте (никакого постоянного NBT).*

## Бижутерия (Curios)
**Файл:** `grimfate/registry/ModCurios.java`

**Кольца:** `copper_ring`, `hunter_band`  
**Амулеты:** `sprouting_amulet`

**Интеграция:**
- Если Curios присутствует — регистрируем типы слотов `ring` (2), `amulet` (1), и читаем статы из наших data-компонентов.
- Если нет — носимка в обычных слотах пока отключена (или «фэйковый» слитый бонус только при наличии Curios).
...
Узлы: **Путь Воина**, **Путь Волшебника**, **Путь Плута** — доступны всем, дают базовые статы и открывают по 1–2 ветки.
...
- **Тип:** ближний конус/дуга перед игроком (анимация сваха; можно позже подключить GeckoLib)
...
- **Тип:** самобуфф + гарантированный бонус к первому удару
